...
import { messagingFeatureForMain } from "@k-lens/messaging-for-main";
import { prometheusFeature } from "@k-lens/prometheus";
...
if (process.env.NODE_ENV !== "production") {
  // 🎯 목적: 개발 모드에서 k-lens 루트 경로를 AppPath/NODE_PATH에 주입해 renderer external 모듈 탐색 보장
  const klensRootDir = process.cwd();

  try {
    app.setAppPath(klensRootDir);
  } catch (error) {
    console.warn("[DEV-NODE-PATH] app.setAppPath 실패", error);
  }

  const desiredPaths = [klensRootDir, path.join(klensRootDir, "node_modules")];
