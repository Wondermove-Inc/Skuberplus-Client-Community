...
import corePackageJson from "@k-lens/core/package.json";
...
const renderer: webpack.Configuration = {
  ...
  entry: {
    globals: require.resolve("@k-lens/core/globals.css"),
    "k-lens": path.resolve(rendererDir, "index.ts"),
  },
  output: {
  ...
  resolve: {
    ...
    alias: {
      ...
      // 📝 변경: k-lens는 @k-lens/core 라이브러리를 사용하므로 core/src/renderer 기준 (2025-10-21)
  ...
  module: {
    ...
    rules: [
      ...
      {
        test: /\.[jt]sx?$/,
        // 🎯 목적: k-lens/src와 workspace packages를 모두 TS 로더로 처리
  ...
  plugins: [
    ...getQuietModeProgressPlugins("k-lens-renderer"),
    new DefinePlugin({
    ...
    new CopyPlugin({
      patterns: [
        {
          from: path.resolve(
            path.dirname(require.resolve("@k-lens/core/package.json")),
            corePackageJson.exports["./fonts"],
          ),
          to: "fonts/[name][ext]",
        },
        {
          from: path.resolve(path.dirname(require.resolve("@k-lens/core/package.json")), "static/build/library/images"),
          to: "images/[name][ext]",
