...
export const appPaths: Partial<Record<NodeJS.Platform, string>> = {
  win32: "./dist/win-unpacked/KLens.exe",
  linux: `./dist/linux${process.arch === "arm64" ? "-arm64" : ""}-unpacked/k-lens`,
  darwin: `./dist/mac${process.arch === "arm64" ? "-arm64" : ""}/KLens.app/Contents/MacOS/KLens`,
};
...
async function getMainWindow(app: ElectronApplication, timeout = 50_000): Promise<Page> {
  return new Promise((resolve, reject) => {
    ...
    const onWindow = (page: Page) => {
      ...
      if (page.url().startsWith("https://renderer.k-lens.app")) {
        cleanup();
    ...
    const timeoutId = setTimeout(() => {
      cleanup();
      console.log(stdoutBuf);
      reject(new Error(`KLens did not open the main window within ${timeout}ms`));
    }, timeout);
...
async function attemptStart() {
  const KLENS_INTEGRATION_TESTING_DIR = path.join(os.tmpdir(), "k-lens-integration-testing", uuid.v4());
  process.env.KLENS_INTEGRATION_TESTING_DIR = KLENS_INTEGRATION_TESTING_DIR;
  ...
  await remove(KLENS_INTEGRATION_TESTING_DIR);
  // We need original .kube/config with kind context
  const testDir = path.join(KLENS_INTEGRATION_TESTING_DIR, "home", ".k-lens", "extensions");
  ...
  try {
    ...
    return {
      ...
      cleanup: async () => {
        try {
          await app.close();
          await withTimeout(remove(KLENS_INTEGRATION_TESTING_DIR), 15_000);
        } catch (_e) {
          // no-op
        }
      },
